...
const PaymentWidget: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 sm:p-6">
      <div className="max-w-lg w-full bg-slate-800/40 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 sm:p-8 animate-fade-in">
        {/* Header */}
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-white mb-2">{paymentData.merchantName}</h1>
          <p className="text-slate-400">{paymentData.description}</p>
        </div>

        {/* Amount */}
        <div className="text-center mb-6 sm:mb-8 p-4 sm:p-6 bg-slate-700/30 rounded-xl border border-slate-600/50">
        ...
        {/* Status */}
        <div className={`flex items-center justify-center space-x-2 sm:space-x-3 p-3 sm:p-4 rounded-xl border mb-4 sm:mb-6 ${statusConfig.bg} ${statusConfig.border}`}>
          <StatusIcon className={`w-6 h-6 ${statusConfig.color}`} />
          <div>
            <p className={`font-semibold text-sm sm:text-base ${statusConfig.color}`}>{statusConfig.title}</p>
            <p className="text-slate-400 text-xs sm:text-sm">{statusConfig.subtitle}</p>
          </div>
        </div>
        ...
        {status === 'waiting' && (
          <>
            ...
            <div className="text-center mb-4 sm:mb-6">
              ...
              <div className="text-xl sm:text-2xl font-bold text-white font-mono">{formatTime(timeLeft)}</div>
            </div>

            {/* QR Code */}
            <div className="mb-4 sm:mb-6">
              <QRCodeGenerator value={paymentData.address} size={window.innerWidth < 640 ? 200 : 250} />
            </div>

            {/* Wallet Address */}
            <div className="mb-4 sm:mb-6">
              <p className="text-slate-400 text-sm mb-2">Or send to wallet address:</p>
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 p-3 bg-slate-700 rounded-xl border border-slate-600">
                <input
                  type="text"
                  value={paymentData.address}
                  readOnly
                  className="flex-1 bg-transparent text-white text-xs sm:text-sm font-mono"
                />
                <button
        ...
        {status === 'confirmed' && (
          <div className="text-center">
            <div className="w-12 h-12 sm:w-16 sm:h-16 bg-green-500 rounded-full flex items-center justify-center mx-auto mb-4 animate-bounce">
